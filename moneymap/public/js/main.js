var AppRouter = Backbone.Router.extend({

    routes: {
        ""                  : "home",
        "entities": "list",
        "entities/page/:page"	: "list",
        "entities/add"         : "addEntity",
        "entities/:id"         : "entityDetails",
        "about"             : "about"
    },

    initialize: function () {
        this.headerView = new HeaderView();
        $('.header').html(this.headerView.el);
    },

    home: function (id) {
        if (!this.homeView) {
            this.homeView = new HomeView();
        }
        $('#content').html(this.homeView.el);
        this.headerView.selectMenuItem('home-menu');
    },

	list: function(page) {
        var p = page ? parseInt(page, 10) : 1;
        var entityList = new EntityCollection();
        entityList.fetch({success: function(){
            $("#content").html(new EntityListView({model: entityList, page: p}).el);
        }});
        this.headerView.selectMenuItem('home-menu');
    },

    entityDetails: function (id) {
        var entity = new Entity({_id: id});
        entity.fetch({success: function(){
            $("#content").html(new EntityView({model: entity}).el);
        }});
        this.headerView.selectMenuItem();
    },

	addEntity: function() {
        var entity = new Entity();
        $('#content').html(new EntityView({model: entity}).el);
        this.headerView.selectMenuItem('add-menu');
	},

    about: function () {
        if (!this.aboutView) {
            this.aboutView = new AboutView();
        }
        $('#content').html(this.aboutView.el);
        this.headerView.selectMenuItem('about-menu');
    }

});

utils.loadTemplate(['HomeView', 'HeaderView', 'EntityView', 'EntityListItemView', 'AboutView'], function() {
    app = new AppRouter();
    Backbone.history.start();
});